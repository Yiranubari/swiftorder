...
export default function OrderPage() {
  ...
  const handleChange = (field, value) => {
    if (
      (field === "orderDetails" || field === "notes" || field === "address") &&
  ...
  const validate = () => {
    ...
    if (!form.customerPhone.trim())
      newErrors.customerPhone = "Your phone number is required";
    if (!form.address.trim())
      newErrors.address = "Delivery address is required";
    if (!form.orderDetails.trim())
      newErrors.orderDetails = "Please describe what you want to order";
    setErrors(newErrors);
        ...
        <div className="flex-1 flex items-center justify-center px-6">
          <div className="text-center max-w-sm animate-fade-in-up">
            ...
            <p className="text-text-secondary font-rubik text-sm mb-6">
              This link is missing a vendor phone number. Please ask the vendor
              for a valid SwiftOrder link.
            </p>
            <Link
              to="/"
      ...
      <main className="flex-1 pt-24 pb-16 px-4 sm:px-6">
        <div className="max-w-lg mx-auto">
          ...
          <div className="bg-surface rounded-2xl border border-border p-5 sm:p-8 shadow-sm animate-fade-in-up delay-100">
            <form onSubmit={handleSubmit} noValidate className="space-y-5">
              <div>
                ...
                />
                {errors.customerName && (
                  <p className="mt-1 text-accent text-xs font-rubik">
                    {errors.customerName}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <div className="flex justify-between mt-1">
                  {errors.address ? (
                    <p className="text-accent text-xs font-rubik">
                      {errors.address}
                    </p>
                  ) : (
                    <span />
                  )}
                  <p className="text-text-muted text-xs font-rubik">
                    {form.address.length}/{MAX_CHARS}
                  </p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex justify-between mt-1">
                  ...
                  <p className="text-text-muted text-xs font-rubik">
                    {form.orderDetails.length}/{MAX_CHARS}
                  </p>
                </div>
              </div>
          ...
          <p className="text-center text-text-muted font-rubik text-xs mt-6 animate-fade-in delay-300">
            Powered by{" "}
            <span className="font-poppins font-semibold text-brand">
              Swift<span className="text-accent">Order</span>
            </span>{" "}
            &mdash; your data goes directly to the vendor's WhatsApp.
          </p>
        </div>
      </main>
